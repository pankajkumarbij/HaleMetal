...
export default function AddCategory() {
  ...
  return (
    <>
      <div className="w-full felx justify-center px-4 pt-20 md:px-52 md:pt-32">
        <div className="border shadow-xl p-6 sm:rounded-lg">
          <p className="text-red-600 text-lg md:text-3xl w-full flex justify-center"><u><b>Add Category</b></u></p>
          <div className="mb-4 mt-6">
            <label className="block text-grey-darker text-sm font-bold mb-2" htmlFor="category">
              Category Name
            </label>
            <input className="shadow appearance-none border rounded w-full py-2 px-3 text-grey-darker" id="category" type="text" placeholder="category name" onChange={(e)=>setCategory(e.target.value)} />
          </div>
          <button className="mb-2 p-2 text-white bg-blue-700 rounded-md hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 w-full" type="button" onClick={()=> NewCategory()} >
            Add Category
          </button>
          {error &&
